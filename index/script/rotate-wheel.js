...
var wedgeColorA = '#ffc107';
var wedgeColorB = '#28a745';
var wedgeColorC = '#20c997';
var wedgeColorD = '#dc3545';
var wedgeSubdiv;
...
// var final_result = document.getElementsByClassName('final-result');
var finalResultElement = document.getElementById("final_result");

var wheel_data;
// var wheel_data = window.wheel_data;
// console.log('window: time='+Date.now());
// console.log(window);
// console.log(wheel_data);
// console.log(wheel_data,'window: time='+ Date.now());
...
function spinWheel() {
  ...
  isResultShown = true;
  wheel_data = window.wheel_data;
  // console.log(wheel_data);
  // console.log("wheel_data"+Date.now());
}
...
function copyLink() {
  ...
  for(c of wheel_data ){
    // console.log(c);
    nameList.push(c.name +" m-"+ c.mid.substr(-2,2));
  }
  refreshList();
}
  ...
  if (rotationSpeed < 0) {
    ...
    if (isResultShown===true) {    
      ...
      if(nameList[selectedIndex]){
        ...
        // console.log(wheel_data);
        // final_result.innerText=nameList[selectedIndex];
        final_result.dataset.sid= sid;
        final_result.dataset.mid= sid+'-'+nameList[selectedIndex].substr(-2,2);
        final_result.innerText="此次抽簽結果。"+nameList[selectedIndex] ;
        // var result_data = document.createElement('span');
        // result_data.innerText = nameList[selectedIndex];
        // final_result.appendChild(result_data);
      };
...
function updateFinalResult(para) {
  finalResultElement.innerText = para;
}
